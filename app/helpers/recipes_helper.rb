module RecipesHelper
  ...
  def link_to_recipe_actions(recipe)
    if current_user == recipe.user
      ...
      link_to("Delete Recipe", user_recipe_path(current_user, recipe), method: :delete, data: { confirm: "Are you sure you want to delete this recipe?" }, class: 'btn btn-danger')
    elsif current_user
      if reviewer_found?
  ...
  def reviewer_found?
    @recipe.reviews.find {|review| review.reviewer == current_user}
  end
